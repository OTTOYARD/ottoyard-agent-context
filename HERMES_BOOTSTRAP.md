...
1. 🚨 **Clone fresh. GitHub is the only source of truth.**
   ...
   **How to work:** clone fresh into your own directory → build and verify → push a `hermes/<lane>/<slug>` (or `hermes/<slug>` when working solo)
...
> **FLEET MODE:** when running as part of the multi-agent fleet, `docs/18_FLEET_COORDINATION.md`
> in the context repo is BINDING LAW and supersedes anything here that conflicts with it —
> including branch naming, migration application, and communication rules.
...
2. Commit it to a branch (hermes/<lane>/<slug> in fleet mode; hermes/<slug> solo)
